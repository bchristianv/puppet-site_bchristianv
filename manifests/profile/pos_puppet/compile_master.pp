...
#   The fully-qualified domain name of the puppet infrastructure MoM (master of masters).
...
#
class site_bchristianv::profile::pos_puppet::compile_master (
  ...
  Stdlib::Fqdn $mom_fqdn = $facts['networking']['fqdn'],
  ...
  class { 'puppetserver':
    ...
    require => [
      Ini_setting['agent-server', 'main-dns_alt_names'],
    ]
  }
  ...
  file { '/etc/puppetlabs/facter/facts.d/pos_infrastructure.json':
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => "{\n\t\"pos_infrastructure\": true\n}\n",
